...
import { Plus, Trash2, Save, Pencil, Clock, GripVertical } from "lucide-react";
...
type Tab = "concours" | "documents" | "articles" | "categories";
...
const emptyCategory = { slug: "", name_ar: "", name_fr: "", icon: "", sort_order: 0 };
    ...
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {(["categories", "concours", "documents", "articles"] as const).map((tab) => (
...
function CategoriesManager() {
  ...
  useEffect(() => {
    supabase.from("categories").select("*").order("sort_order", { ascending: true }).then(({ data }) => { setItems(data || []); setLoading(false); });
  }, []);

  const handleSave = async () => {
    if (!editing) return;
    if (!editing.slug.trim() || !editing.name_ar.trim() || !editing.name_fr.trim()) {
      alert("الرجاء ملء Slug والاسم بالعربية والاسم بالفرنسية");
      return;
    }
    const slug = editing.slug.trim().toLowerCase().replace(/\s+/g, "-");
    if (editing.id.startsWith("new-")) {
      const { data, error } = await supabase.from("categories").insert({ ...editing, slug }).select().single();
      if (error) { alert("Error: " + error.message); return; }
      if (data) setItems((prev) => [...prev.filter((i) => !i.id.startsWith("new-")), data].sort((a, b) => a.sort_order - b.sort_order));
    } else {
      const { error } = await supabase.from("categories").update({ slug: editing.slug, name_ar: editing.name_ar, name_fr: editing.name_fr, icon: editing.icon, sort_order: editing.sort_order }).eq("id", editing.id);
      if (error) { alert("Error: " + error.message); return; }
      setItems((prev) => prev.map((i) => (i.id === editing.id ? { ...editing, slug } : i)).sort((a, b) => a.sort_order - b.sort_order));
    }
    setEditing(null);
  };

  const handleDelete = async (id: string) => {
    if (!confirm("Delete this category? Related concours/documents/articles will keep their category slug.")) return;
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold">الفئات ({items.length})</h2>
        <button onClick={() => setEditing({ ...emptyCategory, id: "new-" + Date.now(), sort_order: items.length })} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 text-white px-4 py-2 text-sm font-medium hover:bg-brand-700">
        ...
        <div className="mb-4 p-4 rounded-lg border border-brand-200 bg-brand-50/50 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input placeholder="e.g. ensa, master" value={editing.slug} onChange={(e) => setEditing({ ...editing, slug: e.target.value })} className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1">الأيقونة (Lucide)</label>
              <input placeholder="e.g. GraduationCap, BookOpen" value={editing.icon || ""} onChange={(e) => setEditing({ ...editing, icon: e.target.value })} className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1">الاسم بالعربية</label>
              <input placeholder="ماستر" value={editing.name_ar} onChange={(e) => setEditing({ ...editing, name_ar: e.target.value })} className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1">Nom en français</label>
              <input placeholder="Master" value={editing.name_fr} onChange={(e) => setEditing({ ...editing, name_fr: e.target.value })} className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </div>
            <div>
              <label className="block text-xs font-medium mb-1">Ordre d'affichage</label>
              <input type="number" value={editing.sort_order} onChange={(e) => setEditing({ ...editing, sort_order: parseInt(e.target.value) || 0 })} className="w-full rounded-lg border border-border px-3 py-2 text-sm" />
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={handleSave} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 text-white px-4 py-2 text-sm font-medium hover:bg-brand-700"><Save className="h-4 w-4" /> حفظ</button>
            <button onClick={() => setEditing(null)} className="px-4 py-2 rounded-lg border border-border text-sm hover:bg-muted">إلغاء</button>
          </div>
        </div>
      )}
        ...
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/50">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-medium">{item.name_ar} <span className="text-muted text-sm">({item.name_fr})</span></p>
              <p className="text-xs text-muted">slug: {item.slug} • icon: {item.icon || "none"} • order: {item.sort_order}</p>
            </div>
...
function ConcoursManager() {
  ...
  const handleSave = async () => {
    if (!editing) return;
    if (!editing.category) { alert("الرجاء اختيار فئة"); return; }
    const { id, ...rawRest } = editing;
    const rest = Object.fromEntries(Object.entries(rawRest).filter(([_, v]) => v !== "" && v !== null && v !== undefined));
    if (editing.id.startsWith("new-")) {
  ...
  return (
    <div>
      ...
      {editing && <ConcoursForm item={editing} onChange={setEditing} onSave={handleSave} onCancel={() => setEditing(null)} categories={categories} setCategories={setCategories} />}
      <div className="space-y-2">
        {items.map((item) => (
          <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/50">
            <div className="flex-1 min-w-0">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full ${item.is_active ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}>{item.is_active ? "Active" : "Inactive"}</span>
            <button onClick={() => setEditing(item)} className="p-1.5 rounded-lg hover:bg-muted"><Pencil className="h-4 w-4" /></button>
            <button onClick={() => handleDelete(item.id)} className="p-1.5 rounded-lg hover:bg-red-50"><Trash2 className="h-4 w-4 text-red-500" /></button>
          </div>
        ))}
...
function ConcoursForm({ item, onChange, onSave, onCancel, categories, setCategories }: { item: ConcoursRow; onChange: (i: ConcoursRow) => void; onSave: () => void; onCancel: () => void; categories: CategoryRow[]; setCategories: React.Dispatch<React.SetStateAction<CategoryRow[]>> }) {
  ...
  const handleAddCategory = async () => {
    if (!newCatSlug.trim() || !newCatNameAr.trim() || !newCatNameFr.trim()) { alert("الرجاء ملء جميع الحقول"); return; }
    setAddingCat(true);
    const slug = newCatSlug.trim().toLowerCase().replace(/\s+/g, "-");
    const { data, error } = await supabase.from("categories").insert({ slug, name_ar: newCatNameAr.trim(), name_fr: newCatNameFr.trim(), sort_order: categories.length + 1 }).select().single();
    setAddingCat(false);
    if (error) { alert("Error: " + error.message); return; }
    if (data) { setCategories((prev) => [...prev, data]); set("category", slug); setNewCatNameAr(""); setNewCatNameFr(""); setNewCatSlug(""); setShowNewCategory(false); }
  };
    ...
    <div className="mb-4 p-4 rounded-lg border border-brand-200 bg-brand-50/50 space-y-3">
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          <CategorySelect value={item.category} onChange={(v) => set("category", v)} categories={categories} />
          {!item.category && <p className="text-xs text-red-500">الرجاء اختيار فئة</p>}
          <button type="button" onClick={() => setShowNewCategory(!showNewCategory)} className="text-xs text-brand-600 hover:text-brand-700 font-medium">+ إضافة فئة جديدة</button>
        </div>
        ...
        <div>
          ...
          {item.deadline && <StatusBadge deadline={item.deadline} />}
        </div>
        <div>
      ...
      {showNewCategory && (
        <div className="p-3 rounded-lg border border-dashed border-brand-300 bg-brand-50/30 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <input placeholder="Slug (e.g. ensa)" value={newCatSlug} onChange={(e) => setNewCatSlug(e.target.value)} className="rounded-lg border border-border px-3 py-2 text-sm" />
            <input placeholder="الاسم بالعربية" value={newCatNameAr} onChange={(e) => setNewCatNameAr(e.target.value)} className="rounded-lg border border-border px-3 py-2 text-sm" />
            <input placeholder="Nom en français" value={newCatNameFr} onChange={(e) => setNewCatNameFr(e.target.value)} className="rounded-lg border border-border px-3 py-2 text-sm" />
          </div>
          <div className="flex gap-2">
            <button onClick={handleAddCategory} disabled={addingCat} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 text-white px-3 py-1.5 text-xs font-medium hover:bg-brand-700 disabled:opacity-50"><Plus className="h-3 w-3" />{addingCat ? "جاري الإضافة..." : "إضافة"}</button>
            <button onClick={() => { setShowNewCategory(false); setNewCatSlug(""); setNewCatNameAr(""); setNewCatNameFr(""); }} className="px-3 py-1.5 rounded-lg border border-border text-xs hover:bg-muted">إلغاء</button>
          </div>
        </div>
      )}
      <div>
      ...
      {editing && (
        <div className="mb-4 p-4 rounded-lg border border-brand-200 bg-brand-50/50 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium mb-1">الفئة</label>
              <CategorySelect value={editing.category} onChange={(v) => setEditing({ ...editing, category: v })} categories={categories} />
            </div>
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/50">
              <div className="flex-1 min-w-0">
                <p className="font-medium truncate">{item.title_fr}</p>
                <p className="text-xs text-muted">{cat ? `${cat.name_ar} (${cat.name_fr})` : item.category} • {item.subject} • {item.year || "N/A"}</p>
              </div>
              <button onClick={() => setEditing(item)} className="p-1.5 rounded-lg hover:bg-muted"><Pencil className="h-4 w-4" /></button>
              <button onClick={() => handleDelete(item.id)} className="p-1.5 rounded-lg hover:bg-red-50"><Trash2 className="h-4 w-4 text-red-500" /></button>
            </div>
        ...
        {items.map((item) => {
          ...
          return (
            <div key={item.id} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/50">
              <div className="flex-1 min-w-0">
                <p className="font-medium truncate">{item.title_fr}</p>
                <p className="text-xs text-muted">{cat ? `${cat.name_ar} (${cat.name_fr})` : item.category || "Uncategorized"} • {item.view_count ?? 0} views</p>
              </div>
              <span className={`text-xs px-2 py-0.5 rounded-full ${item.is_published ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"}`}>{item.is_published ? "Published" : "Draft"}</span>
              <button onClick={() => setEditing(item)} className="p-1.5 rounded-lg hover:bg-muted"><Pencil className="h-4 w-4" /></button>
              <button onClick={() => handleDelete(item.id)} className="p-1.5 rounded-lg hover:bg-red-50"><Trash2 className="h-4 w-4 text-red-500" /></button>
            </div>
          );
